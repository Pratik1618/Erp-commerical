...
const urlParams = new URLSearchParams(window.location.search);
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            fetch(`${api}/material;`,{ headers: {'Authorization': `${token}`} }) // Replace with your actual API endpoint
              .then((response) => response.json())
              .then((data) => {
                const materailSelect = document.getElementById("materailName");
                data.forEach((material) => {
                  const option = document.createElement("option");
                  option.value = material.id;
                  option.textContent = material.material;
                  materailSelect.appendChild(option);
                });
              })
              .catch((error) => {
          ...
          document
            ...
            .addEventListener("submit", function (event) {
              ...
              const materailName = parseInt(document.getElementById("materailName").value);
              ...
              const sqftRate =parseFloat(document.getElementById("sqftRate").value)
              const storeData = {
                tempProposal:{
                  id:stateId
                },
          ...
          function displaySuccess(message) {
            ...
            const table = $("#materail").DataTable({
              ajax: {
                url: `${api}/temp-material/proposalId/${stateId}`, // Replace with your API endpoint
                dataSrc: "",
                headers: {
                  'Authorization': `${token}`
                } 
