...
// }
  ...
  document
    ...
    .addEventListener("submit", function (event) {
      ...
      const salaryHeadCode = document.getElementById("salaryHeadCode").value;
      const salaryHeadName = document.getElementById("salaryHeadName").value;
      const earningAndDeduction = document.getElementById("earningAndDeduction").value;
      const partOfEmployee = document.getElementById("partOfEmployee").value;
      const paySlip = document.getElementById("paySlip").value;
     ...
     // const city=document.getElementById("city").value;
      ...
      const storeData = {
        ...
        earningOrDeduction:earningAndDeduction,
        paySlipOrNonPaySlip:paySlip,
        varriable:variable,
        partOfEmployee:partOfEmployee=='YES' ? 'true': 'false',
      };

      console.log("Form data:", storeData);

      fetch(`${api}/salaryhead/new`, {
        method: "POST",
        headers: {
...
// Initialize DataTables when the document is ready
  ...
  const table = $('#storeTable').DataTable({
      ajax: {
          url: `${api}/salaryhead`, // Replace with your API endpoint
          dataSrc: '',
          headers: {
            'Authorization': `${token}`
          },
      ...
      columns: [
          { data: 'salaryHeadCode' },
          { data: 'salaryHeadName' },
          { data: 'earningOrDeduction' },
          {
            data: 'partOfEmployee',
            render: function(data) {
                return data === true ? 'YES' : 'NO'; // Print YES for true, NO for false
            }
        },
          { data: 'paySlipOrNonPaySlip' },
          { data: 'varriable' },
